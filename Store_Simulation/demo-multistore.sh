...
# Create Glowmark-specific order
print_section "3. Creating Glowmark Order"
GLOWMARK_ORDER_RESPONSE=$(curl -s -X POST "$API_BASE/api/orders/glowmark" \
...
print_success "Created Glowmark order: $GLOWMARK_ORDER_ID"
...
print_section "5. Getting Glowmark Orders for User"
curl -s "$API_BASE/api/orders/glowmark/user/demo-user-001" | jq '.store, .data | map({orderId, status, totalAmount, store})'
...
# Get Glowmark-specific order details
print_section "7. Getting Glowmark Order Details"
curl -s "$API_BASE/api/orders/glowmark/$GLOWMARK_ORDER_ID" | jq '.store, .data | {orderId, userId, status, totalAmount, store}'

# Get Glowmark order status
...
print_section "10. Cancelling Glowmark Order"
CANCEL_RESPONSE=$(curl -s -X PUT "$API_BASE/api/orders/glowmark/$GLOWMARK_ORDER_ID/cancel" \
  -H "Content-Type: application/json" \
  -d '{"reason": "Demo cancellation for Glowmark"}')

print_success "Cancelled Glowmark order: $GLOWMARK_ORDER_ID"
echo $CANCEL_RESPONSE | jq '.store, .data | {orderId, status, store}'

# Get Glowmark-specific statistics
print_section "11. Glowmark Order Statistics"
curl -s "$API_BASE/api/orders/glowmark/stats/summary" | jq '.store, .data'
...
echo "🎯 New Glowmark-specific endpoints:"
echo "   POST   /api/orders/glowmark"
echo "   GET    /api/orders/glowmark/user/:userId"
echo "   GET    /api/orders/glowmark/:orderId"
echo "   GET    /api/orders/glowmark/:orderId/status"
echo "   PUT    /api/orders/glowmark/:orderId/cancel"
echo "   GET    /api/orders/glowmark/stats/summary"
echo ""
