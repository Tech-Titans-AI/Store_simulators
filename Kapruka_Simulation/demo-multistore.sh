...
# Create Kapruka-specific order
print_section "3. Creating Kapruka Order"
GLOWMARK_ORDER_RESPONSE=$(curl -s -X POST "$API_BASE/api/orders/kapruka" \
...
print_success "Created Kapruka order: $GLOWMARK_ORDER_ID"
...
# Get Kapruka-specific user orders
print_section "5. Getting Kapruka Orders for User"
curl -s "$API_BASE/api/orders/kapruka/user/demo-user-001" | jq '.store, .data | map({orderId, status, totalAmount, store})'
...
# Get Kapruka-specific order details
print_section "7. Getting Kapruka Order Details"
curl -s "$API_BASE/api/orders/kapruka/$GLOWMARK_ORDER_ID" | jq '.store, .data | {orderId, userId, status, totalAmount, store}'

# Get Kapruka order status
print_section "8. Getting Kapruka Order Status"
curl -s "$API_BASE/api/orders/kapruka/$GLOWMARK_ORDER_ID/status" | jq '.store, .data | {orderId, status, store, statusHistory: .statusHistory | map(.status)}'

# Try to access Kapuruka order via Kapruka endpoint (should fail)
print_section "9. Testing Store Isolation"
print_info "Attempting to access Kapuruka order via Kapruka endpoint (should fail):"
curl -s "$API_BASE/api/orders/kapruka/$GENERIC_ORDER_ID" | jq '.success, .message'

# Cancel Kapruka order
print_section "10. Cancelling Kapruka Order"
CANCEL_RESPONSE=$(curl -s -X PUT "$API_BASE/api/orders/kapruka/$GLOWMARK_ORDER_ID/cancel" \
  -H "Content-Type: application/json" \
  -d '{"reason": "Demo cancellation for Kapruka"}')

print_success "Cancelled Kapruka order: $GLOWMARK_ORDER_ID"
echo $CANCEL_RESPONSE | jq '.store, .data | {orderId, status, store}'

# Get Kapruka-specific statistics
print_section "11. Kapruka Order Statistics"
curl -s "$API_BASE/api/orders/kapruka/stats/summary" | jq '.store, .data'
...
echo "🎯 New Kapruka-specific endpoints:"
echo "   POST   /api/orders/kapruka"
echo "   GET    /api/orders/kapruka/user/:userId"
echo "   GET    /api/orders/kapruka/:orderId"
echo "   GET    /api/orders/kapruka/:orderId/status"
echo "   PUT    /api/orders/kapruka/:orderId/cancel"
echo "   GET    /api/orders/kapruka/stats/summary"
echo ""
...
echo "   Kapruka:     GLW-timestamp-hash"
